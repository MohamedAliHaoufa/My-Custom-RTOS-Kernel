...
# 18 "bsp/Src/lcd_driver.c" 2
# 26 "bsp/Src/lcd_driver.c"
...
# 75 "bsp/Src/lcd_driver.c"
void LCD_Init(LCD_t* LCD_cfg){
 LCD_GPIO_Init(LCD_cfg);
 MCAL_STK_Delay1ms(2);

 if (LCD_8BIT == LCD_cfg->Mode) {
  LCD_Send_Command(LCD_cfg, (0x38));
  MCAL_STK_Delay1ms(1);
 }
 else if (LCD_4BIT == LCD_cfg->Mode) {
  MCAL_GPIO_WritePin(LCD_cfg->GPIO_PORT, LCD_cfg->RS_PIN, 0);
  MCAL_STK_Delay1ms(1);

  MCAL_GPIO_WritePin(LCD_cfg->GPIO_PORT, LCD_cfg->D4_PIN, ((0x28) & 0x10));
  MCAL_GPIO_WritePin(LCD_cfg->GPIO_PORT, LCD_cfg->D5_PIN, ((0x28) & 0x20));
  MCAL_GPIO_WritePin(LCD_cfg->GPIO_PORT, LCD_cfg->D6_PIN, ((0x28) & 0x40));
  MCAL_GPIO_WritePin(LCD_cfg->GPIO_PORT, LCD_cfg->D7_PIN, ((0x28) & 0x80));
  MCAL_STK_Delay1ms(1);
  LCD_Send_Enable_Signal(LCD_cfg);

  LCD_Send_Command(LCD_cfg, (0x28));
  MCAL_STK_Delay1ms(1);
 }
 LCD_Send_Command(LCD_cfg, LCD_cfg->Display_Mode);
 MCAL_STK_Delay1ms(1);

 LCD_Send_Command(LCD_cfg, (0x01));
 MCAL_STK_Delay1ms(2);

 LCD_Send_Command(LCD_cfg, LCD_cfg->Entry_Mode);
 MCAL_STK_Delay1ms(1);
}
# 115 "bsp/Src/lcd_driver.c"
void LCD_Send_Command(LCD_t* LCD_cfg, uint8 command){
 MCAL_GPIO_WritePin(LCD_cfg->GPIO_PORT, LCD_cfg->RS_PIN, 0);
 MCAL_STK_Delay1ms(1);
...
# 154 "bsp/Src/lcd_driver.c"
void LCD_Send_Char(LCD_t* LCD_cfg, uint8 Char){
 MCAL_GPIO_WritePin(LCD_cfg->GPIO_PORT, LCD_cfg->RS_PIN, 1);
 MCAL_STK_Delay1ms(1);
...
# 228 "bsp/Src/lcd_driver.c"
void LCD_Send_String_Pos(LCD_t* LCD_cfg, uint8 *string, uint8 row, uint8 column){
 LCD_Set_Cursor(LCD_cfg, row, column);
 LCD_Send_String(LCD_cfg, string);
}
# 240 "bsp/Src/lcd_driver.c"
void LCD_Send_Enable_Signal(LCD_t* LCD_cfg){
 MCAL_GPIO_WritePin(LCD_cfg->GPIO_PORT, LCD_cfg->EN_PIN, 1);
 MCAL_STK_Delay1ms(1);
 MCAL_GPIO_WritePin(LCD_cfg->GPIO_PORT, LCD_cfg->EN_PIN, 0);
 MCAL_STK_Delay1ms(1);
}
# 257 "bsp/Src/lcd_driver.c"
void LCD_Set_Cursor(LCD_t* LCD_cfg, uint8 row, uint8 column){
 column--;
