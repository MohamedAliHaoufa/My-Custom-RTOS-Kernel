...
MACH=cortex-m4
CFLAGS= -g -mcpu=$(MACH) -mthumb -mfloat-abi=soft -std=gnu11 -Wall -O0 -I$(INC) -I$(DRIVERS_INC) -I$(BSP_INC) -I$(CMSIS_INC) -I$(RTOS_INC) # Added -I$(INC) -I$(DRIVERS_INC) -I$(BSP_INC) -I$(CMSIS_INC) to include the header files from the inc directories
...
RTOS_FILES=$(wildcard $(RTOS_SRC)/*.c)
...
PPS3=$(patsubst $(RTOS_SRC)/%.c, $(PP)/%.i, $(RTOS_FILES))
ASMS3=$(patsubst $(RTOS_SRC)/%.c, $(ASM)/%.s, $(RTOS_FILES))
OBJS3=$(patsubst $(RTOS_SRC)/%.c, $(OBJ)/%.o, $(RTOS_FILES))

# Build the entire project, generating ELF, hex, and binary files
all: dirs $(OBJS0) $(PPS) $(ASMS) $(OBJS) $(PPS1) $(ASMS1) $(OBJS1) $(PPS2) $(ASMS2) $(OBJS2) $(PPS3) $(ASMS3) $(OBJS3) $(BIN)/$(Project_name).elf $(BIN)/$(Project_name).hex $(BIN)/$(Project_name).bin
	@echo "=======Build is Done============"

# Build with semi-hosting support
semi: dirs $(OBJS0) $(PPS) $(ASMS) $(OBJS) $(PPS1) $(ASMS1) $(OBJS1) $(PPS2) $(ASMS2) $(OBJS2) $(PPS3) $(ASMS3) $(OBJS3) $(BIN)/final_SH.elf
	@echo "=======Build is Done============"
...
# Linking rules
$(BIN)/$(Project_name).elf:  $(OBJS0) $(OBJS) $(OBJS1) $(OBJS2) $(OBJS3)
	$(CC) $(LDFLAGS) -o $@ $^

# Semi-hosting linking rule
$(BIN)/final_SH.elf:  $(OBJS0) $(OBJS) $(OBJS1) $(OBJS2) $(OBJS3)
	$(CC) $(LDFLAGS_SH) -o $@ $^
